...
export async function callGateway(cfg, body, { optimize, fetchImpl = fetch } = {}) {
  ...
  const headers = {
    ...
    // Names this repo as the caller so the run shows up as a proof run on the
    // dashboard rather than as whoever's laptop it happened to execute on.
    'x-anyray-metadata': JSON.stringify({ tool: 'proof-run' }),
  ...
  // The gateway reports what it did in this header. Absent is not an error — a
  // bypassed call has nothing to report, and not every deployment emits it.
  let strategies = [];
  const hdr = res.headers?.get?.('x-anyray-optimization');
  if (hdr) {
    try {
      const decoded = JSON.parse(hdr);
      const list = Array.isArray(decoded) ? decoded : decoded?.decisions || [];
      strategies = list.map((d) => d?.strategy ?? d?.name).filter(Boolean);
    } catch {
      strategies = [];
    }
  }
  return { ...parsed, strategies, latencyMs: Date.now() - started };
}
