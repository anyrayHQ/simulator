...
/**
 * Once the customer has captured their own workloads, the shipped examples stop
 * being useful and start being a problem: they are OUR synthetic fixtures, they
 * cost real provider calls, and they dilute the headline with numbers that are
 * not from the customer's traffic. Found by running SETUP-PROMPT.md end to end
 * — two captured workloads sat in a run of six, so two thirds of the report was
 * ours and 24 of 36 billed calls bought nothing.
 *
 * So: examples run only while they are all there is. `--examples` forces them
 * back in for a demo.
 */
export function partitionExamples(workloads) {
  const captured = workloads.filter((w) => !w.id.startsWith('example-'));
  const examples = workloads.filter((w) => w.id.startsWith('example-'));
  return { captured, examples };
}

export function loadWorkloads(dir = 'workloads', { only = null, examples = false } = {}) {
  ...
  if (only) {
    return { workloads: workloads.filter((w) => w.id === only || w.id.includes(only)), problems };
  }
  const { captured, examples: shipped } = partitionExamples(workloads);
  if (captured.length && !examples) {
    return { workloads: captured, problems, examplesSkipped: shipped.length };
  }
  return { workloads, problems, examplesSkipped: 0 };
}

/**
 * CACHE ISOLATION — why every run stamps a unique id into its prompts.
 *
 * Provider prompt caches outlive a simulator run. Running this repeatedly inside
...
export function stampRunId(workload, runId) {
  const marker = `[anyray-simulator ${runId} — identical in both arms; see README, "Cache isolation"]`;
