...
export function summarize({ results, model, rates, repeats }) {
  const rows = results.map((res) => {
    ...
    return {
      ...
      bypassed,
      optimized,
      direct: armTotals(res.directRuns ?? []),
      cold: {
  ...
  // THE PROXY QUESTION, answered with numbers instead of assurances.
  //
  // Both normal arms traverse Anyray's proxy — one with optimization bypassed —
  // so "without Anyray" really means "Anyray forwarding unchanged". When a
  // direct arm is configured we can say whether that distinction costs
  // anything: the bypassed arm and a genuinely direct call should report the
  // SAME input tokens, because the bypassed path forwards the bytes unaltered.
  // If they differ, that is a finding about us and it belongs at the top of the
  // report, not buried.
  const withDirect = measured.filter((r) => r.direct);
  const proxyCheck = withDirect.length
    ? (() => {
        const direct = withDirect.reduce((a, r) => a + r.direct.billedInput, 0);
        const bypassed = withDirect.reduce((a, r) => a + r.bypassed.billedInput, 0);
        const rows = withDirect.map((r) => ({
          id: r.id,
          direct: r.direct.billedInput,
          bypassed: r.bypassed.billedInput,
          delta: r.bypassed.billedInput - r.direct.billedInput,
        }));
        return {
          workloads: withDirect.length,
          direct,
          bypassed,
          delta: bypassed - direct,
          identical: rows.every((x) => x.delta === 0),
          rows: rows.filter((x) => x.delta !== 0),
        };
      })()
    : null;
...
export function renderRow(r) {
  ...
  const tokens = r.direct
    ? `${n(r.direct.billedInput)} | ${n(r.bypassed.billedInput)} → ${n(r.optimized.billedInput)}`.padEnd(30)
    : `${n(r.bypassed.billedInput)} → ${n(r.optimized.billedInput)}`.padEnd(22);
  ...
  if (r.facts.regression) {
    lines.push(`  ! only missing after the trim: ${r.facts.lost.join(', ')}`);
  }
...
export function renderVerdicts(s) {
  ...
  if (s.proxyCheck) {
    out.push('');
    out.push('Columns: direct to provider | Anyray bypassed → Anyray on');
  }
  out.push('');
  out.push('1 — COST');
  ...
  if (s.proxyCheck) {
    out.push('');
    out.push('   Is the bypassed arm really a baseline?');
    if (s.proxyCheck.identical) {
      out.push(
        `   Yes — on all ${s.proxyCheck.workloads} workload(s) a direct call to your provider reported the SAME input tokens (${n(s.proxyCheck.direct)}) as the call through Anyray with optimization off. The proxy forwards your bytes unaltered.`
      );
    } else {
      out.push(
        `   ! NO — direct ${n(s.proxyCheck.direct)} vs bypassed ${n(s.proxyCheck.bypassed)} (${s.proxyCheck.delta > 0 ? '+' : ''}${n(s.proxyCheck.delta)} through the proxy). The baseline is NOT a clean control, and the saving above is measured against it:`
      );
      for (const x of s.proxyCheck.rows.slice(0, 5)) {
        out.push(`     ${x.id}: direct ${n(x.direct)} → bypassed ${n(x.bypassed)} (${x.delta > 0 ? '+' : ''}${n(x.delta)})`);
      }
    }
  }
  out.push('');
  ...
  if (s.quality.regressions.length) {
    out.push(
      `  ${s.quality.regressions.length} of ${s.quality.checked} checked workloads LOST a required fact after the trim:`
    );
